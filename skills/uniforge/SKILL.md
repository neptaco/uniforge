...
- **Install a GitHub UPM package:** From inside the Unity project, run `uniforge package add <owner/repository/path/to/package>`. Pass an explicit project path before the package source when running elsewhere. The latest semantic-version tag is selected automatically; use `--tag vX.Y.Z` to pin it. Interactive terminals confirm the resolved values; non-interactive runs proceed directly. Use `--yes` to skip a terminal prompt.
