...
- **Update from a package notification:** From inside the Unity project, run `uniforge package update`. Pass a project path when running elsewhere. Before either an Editor-connected update or an offline manifest change, UniForge checks the target release's declared minimum Unity version against `ProjectSettings/ProjectVersion.txt`. Later version streams are accepted when they meet the minimum. A failed or unavailable check stops before the update; use `--force` only for an intentional bypass.
