...
- **Batch mode** — drives Unity in batchmode while the Editor is *closed* (`batch test`, `batch compile`, `batch run`)
- **Live mode** — talks to a *running* Unity Editor through a local daemon (`tool call`, `mcp serve`); requires the Unity-side UPM package
...
|---|---|
| `uniforge doctor unity [project]` | Diagnoses stale Unity runtime state (leftover lock/runtime files, orphan licensing clients) that can block Editor startup or batch mode. Read-only by default. |
| `uniforge doctor unity [project] --fix` | Repairs only verified-stale files; never touches state while a matching Unity process is running. |
...
Typical triage order when "Unity won't start" or "batch mode hangs": `doctor unity` → `logs --trace` → `doctor unity --fix`.
...
```bash
uniforge batch compile <project>
uniforge batch test <project> --platform editmode|playmode
uniforge batch test <project> --platform editmode --filter MyTestClass
uniforge batch test <project> --platform editmode --results ./test-results.xml
uniforge batch run <project> -- -executeMethod Build.Perform -buildTarget StandaloneOSX
```
...
uniforge project open <name>          # partial match
...
uniforge mcp serve                            # MCP server over stdio
```

## Tips

- **Batch mode and an open Editor are mutually exclusive.** If the Editor has the project open, `batch ...` fails with a lock error — use `uniforge tool call` instead (e.g. `run-tests`). Conversely, close the Editor (`uniforge close`) before CI-style batch runs.
- **Choosing a test platform:** type/namespace/asmdef/Editor-extension changes → `editmode`; runtime or UI behavior → `playmode`; just want a compile check → `batch compile`.
...
- **The daemon is self-managing.** `tool call/list/projects` and `mcp serve` auto-start it (`--auto-start-daemon` defaults to true). Only reach for `daemon restart` if `tool projects` shows a stale connection.
...
- **Timeout units differ:** `batch --timeout` is in **seconds**; `tool call --timeout` is in **milliseconds** (default 300000 = 5 min). Raise it for long PlayMode suites.
...
- **Prefer `batch test|compile|run`** — the root-level `test|compile|run` aliases are deprecated.
