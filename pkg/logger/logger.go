...
type Logger struct {
	...
	done             chan struct{} // signals processLogs goroutine has finished
	formatter        *Formatter
	showTime         bool
...
func NewWithOptions(logFile string, opts ...LoggerOption) *Logger {
	...
	// Use os.Pipe for OS-level buffering (typically 64KB on Linux, 16KB on macOS).
	// This prevents Unity process stdout from blocking when processLogs is temporarily slow.
	pr, pw, err := os.Pipe()
	if err != nil {
		// Fallback to io.Pipe (unbuffered) if os.Pipe fails
...
func (l *Logger) processLineCIMode(line string, level LogLevel, noiseCategory NoiseCategory) {
	...
	switch level {
	case LogLevelError:
		l.printLine(fmt.Sprintf("::error::%s", line))
	case LogLevelWarning:
		l.printLine(fmt.Sprintf("::warning::%s", line))
	default:
		l.printLine(line)
	}
}
...
func (l *Logger) processLineNormalMode(line string, level LogLevel) {
	...
	l.printLine(formatted)
}

// printLine writes a single line to stdout, optionally prefixed with a timestamp.
func (l *Logger) printLine(line string) {
	if l.showTime {
		timestamp := time.Now().Format("15:04:05.000")
		_, _ = fmt.Fprintf(os.Stdout, "%s[%s]%s %s\n", ColorGray, timestamp, ColorReset, line)
	} else {
...
func (l *Logger) Close() error {
	...
	// Wait for processLogs goroutine to drain all buffered lines
	if l.done != nil {
		<-l.done
	}
