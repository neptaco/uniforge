...
func (m editorInstallModel) loadAllReleases(currentStreams []VersionStream) tea.Cmd {
	return func() tea.Msg {
		// Try cache first
		cache, err := m.client.LoadCache()
		if err == nil && cache != nil && len(currentStreams) > 0 && m.client.CheckCacheValidity(cache, currentStreams) {
		...
		releases, streams, err := m.client.GetAllReleasesWithStreams()
		if err != nil {
...
func (m editorInstallModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case streamsLoadedMsg:
		...
		if msg.err != nil {
			// Not fatal: continue with an empty stream list; loadAllReleases
			// skips the cache and fetches releases directly
			ui.Debug("Failed to load streams", "error", msg.err)
		}
		m.streams = msg.streams
		m.filteredStreams = msg.streams
		return m, m.loadAllReleases(msg.streams)
