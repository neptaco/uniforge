...
func TestFillGitInfoCountsStagedAndUntrackedChanges(t *testing.T) {
	...
	// Git hook (pre-push など) から起動されると GIT_DIR / GIT_WORK_TREE /
	// GIT_INDEX_FILE が実リポジトリを指したまま子プロセスへ継承され、
	// git -C では上書きされないため実リポジトリの config を破壊する。
	// テンポラリリポジトリへ明示的に固定して隔離する。
	t.Setenv("GIT_DIR", filepath.Join(repoDir, ".git"))
	t.Setenv("GIT_WORK_TREE", repoDir)
	t.Setenv("GIT_INDEX_FILE", filepath.Join(repoDir, ".git", "index"))
