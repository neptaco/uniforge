...
func TestMetaChecker_Check_DoesNotExcludeMatchingNamesInsideAssets(t *testing.T) {
	project, tempDir := setupTestProject(t)
	buildDir := filepath.Join(tempDir, "Assets", "Build")
	if err := os.MkdirAll(buildDir, 0755); err != nil {
		t.Fatalf("Failed to create Assets/Build: %v", err)
	}
	createAssetWithoutMeta(t, buildDir, "Player.cs")
	...
	want := map[string]bool{
		filepath.Join("Assets", "Build"):              false,
		filepath.Join("Assets", "Build", "Player.cs"): false,
	}
	for _, path := range result.MissingMeta {
	...
	for path, found := range want {
		if !found {
			t.Errorf("Expected %s in missing meta list, got: %v", path, result.MissingMeta)
		}
	}
}
...
func TestMetaChecker_Generate_DryRun(t *testing.T) {
	project, tempDir := setupTestProject(t)
	assetsDir := filepath.Join(tempDir, "Assets")

	createAssetWithoutMeta(t, assetsDir, "MissingScript.cs")
	createAssetWithoutMeta(t, assetsDir, "MissingBinary.png")

	checker := NewMetaChecker(project)
	result, err := checker.Generate(true)
	if err != nil {
		t.Fatalf("Generate dry run failed: %v", err)
	}

	if len(result.CreatedMeta) != 1 || result.CreatedMeta[0] != filepath.Join("Assets", "MissingScript.cs.meta") {
		t.Fatalf("unexpected created meta: %v", result.CreatedMeta)
	}
	if len(result.SkippedAssets) != 1 || result.SkippedAssets[0] != filepath.Join("Assets", "MissingBinary.png") {
		t.Fatalf("unexpected skipped assets: %v", result.SkippedAssets)
	}
	if _, err := os.Stat(filepath.Join(assetsDir, "MissingScript.cs.meta")); !os.IsNotExist(err) {
		t.Fatalf("meta file should not be created during dry run")
	}
}

func TestMetaChecker_Generate_WritesSupportedTemplates(t *testing.T) {
	project, tempDir := setupTestProject(t)
	assetsDir := filepath.Join(tempDir, "Assets")
	...
	createAssetWithoutMeta(t, scriptsDir, "Player.cs")

	checker := NewMetaChecker(project)
	result, err := checker.Generate(false)
	if err != nil {
		t.Fatalf("Generate failed: %v", err)
	}

	expected := map[string]bool{
		filepath.Join("Assets", "Scripts.meta"):              true,
		filepath.Join("Assets", "Scripts", "Player.cs.meta"): true,
	}
	for _, path := range result.CreatedMeta {
		delete(expected, path)
	}
	if len(expected) != 0 {
		t.Fatalf("missing expected created meta entries: %v", expected)
	}

	folderMetaPath := filepath.Join(scriptsDir) + ".meta"
	folderMeta, err := os.ReadFile(folderMetaPath)
	if err != nil {
		t.Fatalf("Failed to read folder meta: %v", err)
	}
	if !strings.Contains(string(folderMeta), "folderAsset: yes") {
		t.Fatalf("folder meta did not include folderAsset marker: %s", string(folderMeta))
	}

	scriptMetaPath := filepath.Join(scriptsDir, "Player.cs.meta")
	scriptMeta, err := os.ReadFile(scriptMetaPath)
	if err != nil {
		t.Fatalf("Failed to read script meta: %v", err)
	}
	if !strings.Contains(string(scriptMeta), "MonoImporter:") {
		t.Fatalf("script meta did not include MonoImporter: %s", string(scriptMeta))
	}
	if guid, err := extractGUID(scriptMetaPath); err != nil || len(guid) != 32 {
		t.Fatalf("unexpected generated GUID %q (err=%v)", guid, err)
	}
}
