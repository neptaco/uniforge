...
Examples:
  ...
  uniforge editor available --output json
...
func init() {
	...
	addOutputFlag(editorAvailableCmd, &availableFormat, "Output format: text, json, tsv (auto-detected if not specified)")
	editorAvailableCmd.Flags().BoolVar(&availableLTS, "lts", false, "Show only LTS versions")
	editorAvailableCmd.Flags().StringVar(&availableStream, "stream", "", "Filter by stream: LTS, TECH, BETA, SUPPORTED")
...
func fetchReleasesWithCache(client *hub.Client) ([]hub.UnityRelease, error) {
	...
	type releasesWithStreams struct {
		releases []hub.UnityRelease
		streams  []hub.VersionStream
	}

	// Fetch from API
	result, err := ui.WithSpinner("Fetching available releases...", func() (releasesWithStreams, error) {
		releases, streams, err := client.GetAllReleasesWithStreams()
		return releasesWithStreams{
			releases: releases,
			streams:  streams,
		}, err
	})
	...
	if len(result.streams) > 0 {
		_ = client.SaveCache(result.streams, result.releases)
	}
