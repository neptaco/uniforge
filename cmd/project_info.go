...
func init() {
	...
	projectInfoCmd.Flags().BoolVar(&projectInfoJSON, "json", false, "Output in JSON format")
}
	...
	if err != nil {
		...
		if len(args) > 0 {
			...
			if hubErr == nil && hubProject != nil {
				...
				p, loadErr := unity.LoadProject(hubProject.Path)
				if loadErr != nil {
					return fmt.Errorf("failed to load project: %w", loadErr)
				}
				project = p
			} else {
